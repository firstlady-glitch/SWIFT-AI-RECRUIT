...
export default function EmployerApplicantsPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            // Supabase returns an array or object depending on relationship, user needs to handle type casting
            const processedApps = (appsData || []).map(app => ({
                ...app,
                applicant: Array.isArray(app.applicant) ? app.applicant[0] : app.applicant
            })).filter(app => app.applicant); // Filter out any corrupted records without applicants
    ...
    const handleAnalyzeAI = async () => {
        ...
        try {
            // Filter applicants that need analysis (no score yet)
            // In a real app we might re-analyze all, but for efficiency/cost lets prioritize unscored
            // Or just analyze all to be safe and update scores.
            const appsToAnalyze = applications;

            for (const app of appsToAnalyze) {
                if (!app.applicant) continue;

                // Skip if recently scored to save tokens (optional logic)
            ...
            // Refresh data to show new scores (could be optimized implementation to update local state)
            fetchData();
    ...
    return (
        <div className="min-h-screen bg-[var(--background)] p-8">
            <div className="max-w-7xl mx-auto">
                <Link
                    href={`/app/org/employer/${params.dashboard}/jobs`}
                ...
                <div className="flex flex-col lg:flex-row justify-between lg:items-center gap-6 mb-8">
                    <div>
                        ...
                        <p className="text-gray-400">Manage and rank your applicants</p>
                    </div>
                                ...
                                <div className="lg:w-1/3 border-l border-[var(--border)] pl-0 lg:pl-6 border-t lg:border-t-0 pt-4 lg:pt-0">
                                    ...
                                    {app.match_reasons && app.match_reasons.length > 0 ? (
                                        ...
                                        <p className="text-xs text-gray-500 italic">
                                            Click "Analyze with AI" to generate a match assessment.
                                        </p>
                                    )}
                                </div>
